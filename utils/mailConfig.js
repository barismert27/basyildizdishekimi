...
// Brevo SMTP Transporter Ayarları
const transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST || 'smtp-relay.brevo.com',
    port: process.env.SMTP_PORT || 587,
    secure: false, // 587 portu için genelde false, TLS/STARTTLS kullanır
    auth: {
...
const sendAppointmentEmail = async (ad, email, tarih, saat) => {
    ...
    const mailOptions = {
        from: `"Dr. Muhammed Emin Başyıldız" <${senderEmail}>`,
        to: email,
    ...
    try {
        ...
        console.log('✅ Brevo SMTP üzerinden randevu e-postası başarıyla gönderildi.');
        console.log('📬 Message ID:', info.messageId);
    } catch (error) {
        console.error('❌ Brevo SMTP mail gönderim hatası:', error.message);
    }
};
